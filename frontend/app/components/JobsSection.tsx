...
export default function JobsSection({
  ...
  const loadProfiles = useCallback(async () => {
    try {
      const res = await authFetch(`/programs/${programId}/scan-profiles`);
      if (!res.ok) return;
      const data = await res.json();
  ...
  async function queuePipeline() {
    try {
      const res = await authFetch(`/programs/${programId}/pipelines`, {
        method: "POST",
        body: JSON.stringify({
          stages: [
  ...
  async function previewJob(spec: { tool: string; source: string; config: Record<string, unknown> }): Promise<JobPreview | null> {
    try {
      const res = await authFetch(`/programs/${programId}/jobs/preview`, {
        method: "POST",
  ...
  async function deleteProfile(id: string) {
    try {
      const res = await authFetch(`/programs/${programId}/scan-profiles/${id}`, { method: "DELETE" });
