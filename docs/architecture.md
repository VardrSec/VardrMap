...
3. **Composer** (`jobs/Composer.tsx`) — a single selection across the recon pipeline and the four tools (subfinder/httpx/nuclei/nmap). Picking the pipeline clears the tool highlight and vice versa, so only one thing is ever chosen; the tool's config is preserved across the toggle. Selecting the pipeline swaps the per-tool config for a read-only stage list and hides target source, recurrence, and saved profiles — stages carry their own source and config, and `/schedules` is single-tool only, so a recurring pipeline would silently drop its later stages. Nothing is queued until **Queue**, and the pipeline confirms first since it queues three jobs. The chain itself lives in `RECON_PIPELINE` (`jobs/mockData.tsx`) and is shared with `JobsSection`, so what is displayed is exactly what is posted.
