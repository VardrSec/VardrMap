...
- **Pipelines** (`POST /programs/{id}/pipelines`) create an ordered chain of `scan_jobs` linked by `depends_on`. A dependent stage is withheld from `GET /jobs/pending` until its parent is `done`; the runner's own poll drives the clock (same pattern as scheduled scans). If a parent `failed`, `GET /jobs/pending` auto-fails the dependent stage so it never hangs. The endpoint accepts any valid ordered chain. The UI offers subfinder → httpx → nuclei from `RECON_PIPELINE` (`frontend/app/components/jobs/mockData.tsx`) with each stage individually includable, so it may post any ordered subset of those three — `depends_on` is linked sequentially over whatever arrives, so a subset still chains correctly.
...
3. **Composer** (`jobs/Composer.tsx`) — a single selection across the recon pipeline and the four tools (subfinder/httpx/nuclei/nmap). Picking the pipeline clears the tool highlight and vice versa, so only one thing is ever chosen; the tool's config is preserved across the toggle. Nothing is queued until **Queue**, and the pipeline confirms first since it queues several jobs at once.

   Selecting the pipeline expands it into a **stage editor**: each stage from `RECON_PIPELINE` can be included or excluded, so subfinder → httpx alone is as valid as the full chain. Order always follows `RECON_PIPELINE`, so re-including a stage restores its original position rather than appending it. The card blurb, the summary line and the confirm dialog all read from the current selection, so none of them can advertise a chain the operator has edited away. Queue is disabled while zero stages are included.
   ...
   `RECON_PIPELINE` (`jobs/mockData.tsx`) is the single definition; `JobsSection` posts the stage list the Composer hands back, so what is displayed is exactly what is queued.
